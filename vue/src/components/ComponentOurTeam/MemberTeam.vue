<template>
  <div class="mb-8">
      ...
      <div class="grid grid-cols-5 gap-8 px-16 my-4">
      ...
      <div v-for="category in listCategory" :key="category.id">
          <div class="grid grid-cols-2 gap-8" v-if="visibleMembers[category.id - 1] == true">
              <div v-for="member in category.members" :key="member.id">
                <img class="m-auto w-5/12" :src="member.image" alt="">
                <h1 class="font-bold text-4xl text-center pt-4">{{member.member}}</h1>
                <p class="text-2xl text-center">{{member.division}}</p>
              </div>
          </div>
      </div>
  </div>
